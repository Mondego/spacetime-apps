...
import logging
from datamodel.search.datamodel import AllLink, BadUrlPattern
...
LOG_HEADER = "[DOWNLOADEDSAVER]"
FINAL_FOLDER = "DOWNLOAD"

@Getter(AllLink, BadUrlPattern)
class DownloadedSaver(IApplication):
    def __init__(self, frame):
        self.frame = frame

    def initialize(self):
        self.save_bad_patterns(self.frame.get(BadUrlPattern))
        self.save_links(self.frame.get(AllLink))

    def update(self):
        self.save_bad_patterns(self.frame.get_new(BadUrlPattern) + self.frame.get_mod(BadUrlPattern))
        self.save_links(self.frame.get_new(AllLink) + self.frame.get_mod(AllLink))
    
    def shutdown(self):
        pass

    def save_bad_patterns(self, patterns):
        if not os.path.exists(FINAL_FOLDER):
            try:
                os.makedirs(FINAL_FOLDER)
            except Exception:
                print "Exception"
        with open(os.path.join(FINAL_FOLDER, "bad_patterns.txt"), "a") as bp:
            for p in patterns:
                bp.write(p.pattern + "\n")
